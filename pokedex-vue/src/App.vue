<template>
  ...
  >
    <section class="pokecard">
      <div class="nomeImg">
        <h1 class="pokemonNome">{{ pokemonData.name }}</h1>
        <img 
...
export default {
  ...
  methods: {
    async procurarPoke () {
      try {
        const pokeEnviar = await fetch(`${pokeapi}/${this.pokemonID}`)
        const pokemon = await pokeEnviar.json()
...
@import './poke_tipos.scss';
@import url('https://fonts.googleapis.com/css2?family=Changa:wght@400;700&display=swap');
.header, .main, input[type="text"], .procurarPoke {
  font-family: 'Changa', sans-serif;
}
.header, input[type="text"], .procurarPoke {
  font-size: 1.5rem;
}
